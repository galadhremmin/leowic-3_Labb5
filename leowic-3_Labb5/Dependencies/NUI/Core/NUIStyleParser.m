...
- (NUIStyleSheet *)parse:(NSString *)styles
{
    ...
    NSCharacterSet *hexIdCharacters = [NSCharacterSet characterSetWithCharactersInString:@"0123456789ABCDEF"];
    NSCharacterSet *hexInitialIdCharacters = [NSCharacterSet characterSetWithCharactersInString:@"#"];
    [tokeniser addTokenRecogniser:[CPIdentifierRecogniser identifierRecogniserWithInitialCharacters:hexInitialIdCharacters identifierCharacters:hexIdCharacters]];
    ...
    NSString *expressionGrammar =
       @"NUIStyleSheet            ::= definitions@<NUIDefinition>* ruleSets@<NUIRuleSet>*;\n"
        ...
        "NUIValue                 ::= <NUIAny>+;\n"
        "NUIAny                   ::= 'Identifier' | 'Number' | 'Variable' | '(' | ')' | ',';\n"
        ;
